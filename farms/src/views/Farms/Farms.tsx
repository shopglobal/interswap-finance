...
import { useFarms, usePriceBnbBusd, usePriceCakeBusd, usePriceEthusd, usePriceCakeOGBusd, usePriceATOMusd, usePriceCRYSTAL, usePriceAQUA } from 'state/hooks'
...
const Farms: React.FC = () => {
  ...
  const aquaPrice = usePriceAQUA()
  const crystalPrice = usePriceCRYSTAL()
  ...
  const farmsList = useCallback(
    (farmsToDisplay): FarmWithStakedValue[] => {
      ...
      let farmsToDisplayWithAPY: FarmWithStakedValue[] = farmsToDisplay.map((farm) => {
        ...
        //   apy = cakeOGPrice.times(cakeRewardPerYear).div(farm.lpTotalInQuoteToken).div(bnbPrice)
        // }
        //   if (farm.quoteTokenSymbol === QuoteToken.ATOM) {
        //   apy = atomPrice.times(cakeRewardPerYear).div(farm.lpTotalInQuoteToken).div(20)
        // }
        //   if (farm.quoteTokenSymbol === QuoteToken.BNB) {
        //   apy = cakePriceVsBNB.times(cakeRewardPerYear).div(farm.lpTotalInQuoteToken).times(10)
        // }
        //   if (farm.quoteTokenSymbol === QuoteToken.ETH) {
        //   apy = ethPrice.times(cakeRewardPerYear).div(farm.lpTotalInQuoteToken).div(bnbPrice)
        // }
          if (farm.pid === 1) {
            apy = cakePriceVsBNB.times(cakeRewardPerYear).div(farm.lpTotalInQuoteToken).times(bnbPrice)
        } 
          if (farm.pid === 2) {
            apy = cakeRewardPerYear.div(farm.lpTotalInQuoteToken).times(cakeOGPrice)
        } 
          if (farm.pid === 3) {
            apy = cakePriceVsBNB.times(cakeRewardPerYear).div(farm.lpTotalInQuoteToken).times(bnbPrice)
        }  
          if (farm.pid === 4) {
            apy = cakeRewardPerYear.div(farm.lpTotalInQuoteToken).times(ethPrice)
        }  
          if (farm.pid === 5) {
            apy = cakeRewardPerYear.div(farm.lpTotalInQuoteToken).times(atomPrice)
        }  
          if (farm.pid === 6) {
            apy = cakeRewardPerYear.div(farm.lpTotalInQuoteToken).times(cakePrice)
        }  
          if (farm.pid === 7) {
            apy = cakeRewardPerYear.div(farm.lpTotalInQuoteToken).times(cakePrice)
        }  
          ...
          if (farm.pid === 11) {
            apy = cakeRewardPerYear.div(farm.lpTotalInQuoteToken).times(crystalPrice)
        }  
        if (farm.pid === 12) {
          apy = cakePriceVsBNB.times(cakeRewardPerYear).div(farm.lpTotalInQuoteToken).times(bnbPrice)
        }
          ...
          if (farm.pid === 17) {
            console.log(Number(aquaPrice))
          apy = cakeRewardPerYear.div(farm.lpTotalInQuoteToken).times(aquaPrice)
        }    
          if (farm.dual) {
          const cakeApy =
        ...
        if (farm.quoteTokenSymbol === QuoteToken.CAKE_OG) {
          liquidity = cakeOGPrice.times(farm.lpTotalInQuoteToken)
        }
        if (farm.quoteTokenSymbol === QuoteToken.ETH_OG || farm.quoteTokenSymbol === QuoteToken.ETH) {
          liquidity = ethPrice.times(farm.lpTotalInQuoteToken)
        }
        if (farm.quoteTokenSymbol === QuoteToken.ATOM) {
          liquidity = atomPrice.times(farm.lpTotalInQuoteToken).times(100)
        }
        if (farm.quoteTokenSymbol === QuoteToken.ETH) {
          liquidity = ethPrice.times(farm.lpTotalInQuoteToken)
        }
    ...
    [bnbPrice, farmsLP, query, cakePrice, ethPrice, cakeOGPrice, atomPrice, aquaPrice, crystalPrice],
  )
  ...
  const rowData = farmsStaked.map((farm) => {
    const { quoteTokenAdresses, quoteTokenSymbol, tokenAddresses, quoteContract } = farm
    ...
    <>
      <Header><div className="text">
        ...
        <Heading size="lg" color="text">
          {TranslateString(999, 'Stake Liquidity Pool (LP) tokens to earn Electronero Smart Chain (XSC) rewards tokens on Binance Smart Chain with every block that is mined!')}
        </Heading></div>
      </Header>
      <Page> 
      <RocketContainer>       
