...
import useGetPriceCRYSTALData from '../components/Menu/getPricesCRYSTAL'
import useGetPriceAQUAData from '../components/Menu/getPricesAQUA'
...
export const usePriceCRYSTAL = (): BigNumber => {
  // const pid = 4 // XSC-ETH LP
  // @ts-ignore
  const priceDataCRYSTAL = useGetPriceCRYSTALData()

  // @ts-ignore
  const crystalPrice = new BigNumber(priceDataCRYSTAL)

  return crystalPrice;

}
export const usePriceCRYSTALusd = (): BigNumber => {
  const pid = 11;
  const priceETH = usePriceCRYSTAL()
  const farm = useFarmFromPid(pid)
  return farm.tokenPriceVsQuote ? priceETH.times(farm.tokenPriceVsQuote) : ZERO
}

export const usePriceAQUA = (): BigNumber => {
  // const pid = 4 // XSC-ETH LP
  // @ts-ignore
  const priceDataAQUA = useGetPriceAQUAData()

  // @ts-ignore
  const aquaPrice = new BigNumber(priceDataAQUA)

  return aquaPrice;

}
export const usePriceAQUAusd = (): BigNumber => {
  const pid = 17;
  // const bnbPriceUSD = usePriceBnbBusd()

  let priceData = useGetPriceAQUAData()
